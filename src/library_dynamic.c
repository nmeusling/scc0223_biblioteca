...
void create_stud_list(library *lib) {
    lib->students.first = NULL;
    lib->students.last = NULL;
}
...
int insert_student(library *lib, char name[MAX_NAME_SIZE],
    ...
    if (lib->students.first == NULL) {
        lib->students.first = s;
    } else { //list has at least one element already
        lib->students.last->next = s;
    }
    lib->students.last = s;
...
int remove_student_name(library *lib, char name[MAX_NAME_SIZE]) {
    ...
    student *stud = lib->students.first;
    //list is empty
    if (stud == NULL) {
        return 1;
    }
    ...
    remove_student_all_waitlists(&lib->books, stud);
    free(stud);
    return 0;
}
...
int insert_book(library *lib, char title[MAX_TITLE_SIZE],
    ...
    //if list is currently empty
    if (lib->books.first == NULL) {
        lib->books.first = b;
    } else { //list has at least one element already
        lib->books.last->next = b;
    }
    lib->books.last = b;
    b->count = 1;
...
int remove_book_title(library *lib, char title[MAX_TITLE_SIZE]) {
    ...
    if (search_book_title(lib, title, &prev_book) == 1) {
        return 1;
    }
    //book is 1st element
    if (prev_book == NULL) {
        lib->books.first = book->next;
...
int remove_book_isbn(library *lib, int isbn[MAX_ISBN_SIZE]) {
    ...
    //student not found in list
    if (search_book_isbn(lib, isbn, &prev_book) == 1) {
        return 1;
    }
    //student is 1st element
    if (prev_book == NULL) {
...
int get_student_by_name(library *lib, char name[MAX_NAME_SIZE], student **stud){
    student *prev ;
    if(search_student_name(lib, name, &prev) == 1)
        return 1;
    if(prev == NULL)
        *stud = (lib->students.first);
    else
        *stud = (prev)->next;
    return 0;
}
...
int get_student_by_nusp(library *lib, int nusp[MAX_NUSP_SIZE], student **stud){
    student *prev ;
    if(search_student_nusp(lib, nusp, &prev) == 1)
        return 1;
    if(prev == NULL)
        *stud = lib->students.first;
    else
